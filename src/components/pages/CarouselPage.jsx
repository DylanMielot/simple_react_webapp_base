...
export function TitlePage({ children }) {
    return <Centered className="w-full h-[60px]">
        <h1 className="w-[60%] text-center pt-[10px] h-full text-[120%] border-b-[1px] border-black">
            {children}
        </h1>
    </Centered>
}
...
export function ContentPage({ children }) {
    return <div className=" w-full h-full overflow-y-auto overflow-x-hidden p-5">
        {children}
    </div>
}
...
export default function CarouselPage({ children, pages }) {
    ...
    try {
        ...
        console.error(e)
        var content = <Centered className="h-full font-bold">{e.message}</Centered>
    }
